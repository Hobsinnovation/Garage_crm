...
class Booking {
    ...
    public function generate($company, $project, $client, $partslist, $save = false, $signature = NULL) {
        ...
        // Document Title
        $pdf->Image(document_asset("assets/images/headerpattern.png"), 0, $yPos + 1, 320);

        $pdf->SetFillColor(7, 22, 47);
        ...
        $pdf->SetTextColor(128, 148, 174);
        if (!empty($project->booking_signature) && !empty($project->signed_by)) {
            $pdf->Image(document_asset("uploads/bookingsignatures/".$project->booking_signature), $xPos, $yPos + 15, 80);
        ...
        if (!empty($company->logo)) {
            $pdf->Image(document_asset("uploads/logos/".$company->logo), $xPos + 40, $yPos, 160);
        }else{
            $pdf->SetFont('','B',24);
        ...
        if($company->car_diagram == "Enabled" && !empty($project->car_diagram)){
            ...
            $yPos = $pdf->GetY() + 20;
            $pdf->SetXY($xPos, $yPos);
            $pdf->Image(document_asset("uploads/cardiagrams/".$project->car_diagram), $xPos, $yPos, 500, 250);
...
class PDF extends TCPDF {
    ...
    public function Footer() {
        ...
        $this->Image(document_asset("assets/images/docpattern.png"), 0, $yPos, 680);
