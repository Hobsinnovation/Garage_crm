    ...
    public function findRoute(string $name): ?ILoadableRoute
    {
        ...
        foreach ($this->processedRoutes as $route) {
            ...
            if (is_string($callback) === true && is_callable($callback) === false && strpos($name, '@') !== false && strpos($callback, '@') !== false) {
                 ...
                 * Do NOT prefix-match controller methods: Invoice@render must not
                 * resolve to Invoice@renderstatement. Prefix matching breaks URL
                 * generation whenever one action name begins with another. */
                if (strtolower($callback) === strtolower($name)) {
