                                        ...
                                        <div class="card-aside-wrap">
                                            <div class="card-content">
                                                <ul class="nav nav-tabs nav-tabs-mb-icon nav-tabs-card">
                                                    ...
                                                    <li class="nav-item">
                                                    <a class="nav-link
<?php if (isset($_GET['view']) && $_GET['view'] == 'statement') { ?>
 active
<?php } ?>
" href="<?=  url('Clients@details', array('clientid' => $client->id)) ; ?>?view=statement">
                                                        ...
                                                        <div class="bq-note">
                                                            ...
                                                            <div class="bq-note-item">
                                                                ...
                                                                <div class="bq-note-meta">
                                                                    ...
                                                                    <span class="bq-note-sep sep">•</span>
                                                                    <a href="<?=  app_url('jobcards/'.$jobcard->id.'/render') ; ?>" class="link link-sm link-primary" download="Job card #<?=  $jobcard->id ; ?>">Download Job Card</a>
                                                                    <?php if ($user->role == "Owner" || $user->role == "Admin") { ?>
                                                                    <span class="bq-note-sep sep">•</span>
                                                                        ...
                                                                        <li>
                                                                            <div class="drodown">
                                                                                ...
                                                                                <div class="dropdown-menu dropdown-menu-right">
                                                                                    ...
                                                                                    <li><a href="<?=  app_url('quotes/'.$quote->id.'/render') ; ?>" download="Quote #<?=  $quote->id ; ?>.pdf"><em class="icon ni ni-download-cloud"></em><span>Download</span></a></li>
...
<?php } else if (isset($_GET["view"]) && $_GET["view"] == "statement") { ?>
<div class="card-inner">
    <div class="nk-block mb-2">
        <div class="nk-block-head">
            <h5 class="title">Client Account Statement</h5>
            <p>Account statement for <?=  $client->fullname ; ?>.</p>
        </div>
    </div>

<form method="GET" action="<?=  url('Clients@details', array('clientid' => $client->id)) ; ?>">
        <input type="hidden" name="view" value="statement">
        <div class="row gy-3 mb-4 align-items-end">
            <div class="col-sm-4">
                <div class="form-group">
                    <label class="form-label">From Date</label>
                    <div class="form-control-wrap">
                        <input type="date" class="form-control form-control-lg" name="from"
                            value="<?=  !empty($_GET['from']) ? $_GET['from'] : date('Y-m-d', strtotime('-6 months')) ; ?>">
                    </div>
                </div>
            </div>
            <div class="col-sm-4">
                <div class="form-group">
                    ...
                    <div class="form-control-wrap">
                        <input type="date" class="form-control form-control-lg" name="to"
                            value="<?=  !empty($_GET['to']) ? $_GET['to'] : date('Y-m-d') ; ?>">
                    </div>
                </div>
            </div>
            <div class="col-sm-4">
                <button type="submit" class="btn btn-primary btn-lg">
                    <em class="icon ni ni-search"></em> <span>Generate Statement</span>
                </button>
            </div>
        </div>
    </form>
    ...
    <div class="row g-3 mb-4">
        <div class="col-sm-3">
            <div class="card card-bordered text-center p-3">
                <p class="text-soft mb-1">Total Invoiced</p>
                <h5 class="text-primary"><?=  money($statement_total, $user->parent->currency) ; ?></h5>
            </div>
        </div>
        <div class="col-sm-3">
            <div class="card card-bordered text-center p-3">
                <p class="text-soft mb-1">Total Paid</p>
                <h5 class="text-success"><?=  money($statement_paid, $user->parent->currency) ; ?></h5>
            </div>
        </div>
        <div class="col-sm-3">
            <div class="card card-bordered text-center p-3">
                <p class="text-soft mb-1">Outstanding Balance</p>
                <h5 class="text-danger"><?=  money($statement_balance, $user->parent->currency) ; ?></h5>
            </div>
        </div>
        <div class="col-sm-3">
            <div class="card card-bordered text-center p-3">
                <p class="text-soft mb-1">Total Invoices</p>
                <h5 class="text-dark"><?=  count($statement_invoices) ; ?></h5>
            </div>
        </div>
    </div>

    <div class="mb-3">
        <a href="<?=  app_url('clients/'.$client->id.'/statement/pdf') ; ?>?from=<?=  $_GET['from'] ; ?>&to=<?=  $_GET['to'] ; ?>"
            class="btn btn-dim btn-outline-primary" target="_blank">
            <em class="icon ni ni-download-cloud"></em> <span>Download PDF Statement</span>
        </a>
    </div>

    <?php if (!empty($statement_invoices)) { ?>
    <table class="datatable-init nk-tb-list nk-tb-ulist mt" data-auto-responsive="false">
        <thead>
            <tr class="nk-tb-item nk-tb-head">
                <th class="nk-tb-col text-center">#</th>
                <th class="nk-tb-col tb-col-md"><span class="sub-text">Invoice #</span></th>
                <th class="nk-tb-col tb-col-md"><span class="sub-text">Vehicle / Reg No</span></th>
                <th class="nk-tb-col tb-col-md"><span class="sub-text">Invoice Date</span></th>
        ...
        <tbody>
            <?php foreach ($statement_invoices as $index => $inv) { ?>
            <tr class="nk-tb-item">
                <td class="nk-tb-col text-center"><?=  $index + 1 ; ?></td>
                <td class="nk-tb-col tb-col-md">
                    <span class="tb-lead">INV-<?=  str_pad($inv->id, 4, '0', STR_PAD_LEFT) ; ?></span>
                </td>
                <td class="nk-tb-col tb-col-md">
                    <?php if (!empty($inv->project)) { ?>
                        <span class="tb-lead"><?=  carmake($inv->project->make) ; ?> <?=  carmodel($inv->project->model) ; ?></span>
                        <span><?=  $inv->project->registration_number ; ?></span>
                    <?php } else { ?>
                        <span>--</span>
                    <?php } ?>
                </td>
                <td class="nk-tb-col tb-col-md">
                    <span><?=  date("M j, Y", strtotime($inv->invoice_date)) ; ?></span>
                </td>
                <td class="nk-tb-col tb-col-md">
                    <?php if ($inv->due_date < date("Y-m-d") && $inv->status != "Paid") { ?>
                        <span class="text-danger"><?=  date("M j, Y", strtotime($inv->due_date)) ; ?></span>
                    <?php } else { ?>
                        <span><?=  date("M j, Y", strtotime($inv->due_date)) ; ?></span>
                    <?php } ?>
                </td>
                <td class="nk-tb-col">
                    <span class="tb-amount"><?=  money($inv->total, $user->parent->currency) ; ?></span>
                </td>
                <td class="nk-tb-col">
                    <span class="text-success"><?=  money($inv->amount_paid, $user->parent->currency) ; ?></span>
                </td>
                <td class="nk-tb-col">
                    <span class="text-danger"><?=  money($inv->balance, $user->parent->currency) ; ?></span>
                </td>
                <td class="nk-tb-col tb-col-md">
                    <?php if ($inv->status == "Paid") { ?>
                        <span class="badge badge-sm badge-dot has-bg badge-success d-none d-mb-inline-flex">Paid</span>
                    <?php } else if ($inv->status == "Partial") { ?>
                        <span class="badge badge-sm badge-dot has-bg badge-warning d-none d-mb-inline-flex">Partial</span>
                    <?php } else { ?>
                        <span class="badge badge-sm badge-dot has-bg badge-danger d-none d-mb-inline-flex">Unpaid</span>
                    <?php } ?>
                ...
                <td class="nk-tb-col nk-tb-col-tools">
                    <ul class="nk-tb-actions gx-1">
                        <li>
                            <a href="<?=  url('Invoice@view', array('invoiceid' => $inv->id)) ; ?>"
                                class="btn btn-icon btn-trigger btn-sm" data-toggle="tooltip" title="View Invoice">
                                <em class="icon ni ni-eye"></em>
                            </a>
                        </li>
                        <li>
                            <a href="<?=  app_url('invoices/'.$inv->id.'/render') ; ?>"
                                class="btn btn-icon btn-trigger btn-sm" download="Invoice #<?=  $inv->id ; ?>.pdf"
        ...
        <tfoot>
            <tr class="nk-tb-item" style="background:#f5f6fa; font-weight:bold;">
                <td class="nk-tb-col text-center" colspan="5"><strong>TOTALS</strong></td>
                <td class="nk-tb-col"><strong><?=  money($statement_total, $user->parent->currency) ; ?></strong></td>
                <td class="nk-tb-col text-success"><strong><?=  money($statement_paid, $user->parent->currency) ; ?></strong></td>
                <td class="nk-tb-col text-danger"><strong><?=  money($statement_balance, $user->parent->currency) ; ?></strong></td>
